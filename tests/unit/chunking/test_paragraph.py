"""Paragraph chunker + ``anchor_chunk_for_position``."""
...
def test_short_text_is_one_chunk() -> None:
    chunks = chunk_paragraph("hello world")
    assert len(chunks) == 1
...
def test_long_text_yields_multiple_chunks_with_overlap() -> None:
    text = "x" * 6000
    chunks = chunk_paragraph(text, target_chars=2000, overlap_chars=200)
    assert len(chunks) >= 3
...
def test_offsets_are_consistent_with_text() -> None:
    text = "alpha\n\n" + ("beta\n\n" * 200) + "gamma"
    chunks = chunk_paragraph(text, target_chars=300, overlap_chars=50)
    assert len(chunks) > 1
    for c in chunks:
...
def test_chunker_prefers_paragraph_boundary() -> None:
    text = "a" * 1500 + "\n\n" + "b" * 1500
    chunks = chunk_paragraph(text, target_chars=2000, overlap_chars=100)
